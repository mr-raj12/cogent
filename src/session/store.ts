...
export function getSessionDir(): string {
	return join(homedir(), ".pi-clone", "sessions");
}
...
export async function appendEntry(sessionId: string, entry: SessionEntry): Promise<void> {
	// TODO PHASE 5 — Append a single JSON line to the session file
	//
...
export async function loadSession(sessionId: string): Promise<Session | null> {
	// TODO PHASE 5 — Read a JSONL file and reconstruct a Session
	//
...
export async function listSessions(): Promise<string[]> {
	// TODO PHASE 5 — List all session IDs
	//
